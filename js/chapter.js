  ...
  if (cardModal) {
      cardModal.addEventListener('show.bs.modal', async function() {
          if (currentCardData && currentCardData.id) {
              ...
              await displayChapters(currentCardData.id);
          }
      });
  }
...
async function displayChapters(cardId) {
  ...
  try {
      const response = await fetch(`http://localhost:3000/api/chapters/${cardId}`);
      ...
      chapters.forEach((chapter, index) => {
          ...
          accordionButton.setAttribute('data-bs-target', `#collapseChapter${chapter.chapter_number}`);
          accordionButton.setAttribute('aria-expanded', index === 0 ? 'true' : 'false');
          accordionButton.setAttribute('aria-controls', `collapseChapter${chapter.chapter_number}`);
          accordionButton.textContent = `Chương ${chapter.chapter_number}`;

          const accordionCollapse = document.createElement('div');
          accordionCollapse.id = `collapseChapter${chapter.chapter_number}`;
          ...
          chapterContent.innerHTML = `<strong>${chapter.chapter_title}</strong> ${chapter.content || ''}`;
...
// Giữ nguyên các hàm khác như openReadModal, setupChapterModalBehavior
